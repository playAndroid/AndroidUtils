...
public class CalendarUtils {
    ...
    private static int leapMonth(int year) {
    	int i=0;
    	try {
			i=(int) (LUNAR_INFO[year - 1900] & 0xf);
		} catch (Exception e) {
    ...
    public String toString() {
        ...
        try {
			if (StringUtils.isNullOrEmpty(message)) {
			    String solarMsg = new SolarTermsUtils(mCurrenCalendar).getSolartermsMsg();
			    //判断当前日期是否为节气
			    if (!StringUtils.isNullOrEmpty(solarMsg)) {
			        message = solarMsg;
			    } else {
			        /**
			         * 判断当前日期是否为公历节日
			         */
			        String gremessage = new GregorianUtils(mCurrenCalendar).getGremessage();
			        if (!StringUtils.isNullOrEmpty(gremessage)) {
			            message = gremessage;
			        } else if (mLuchDay == 1) {
    ...
    public String getDay() {
    	String string="";
    	 try {
             string=(isLoap ? "闰" : "") + CHINESE_NUMBER[mLuchMonth - 1] + "月"
     ...
     *
     * @param calendar Calendar
     * @return 返回成转换好的  转换为2012年11月22日格式
     */
    public static String changeToChineseDay(Calendar calendar) {
     ...
     */
    public static Date getChineseDate(String string) throws ParseException {
        return chineseDateFormat.parse(string);
    }
    
    /**
     * 把calendar转化为当前 HH:mm
     *
     * @param calendar Calendar
     * @return 返回成转换好的 年月日格式
     */
    ...
    public static String changDate(String string) {
		if (string != null && string.contains(" ") && string.contains("-")) {
			return string.split(" ")[0].split("-")[0] + "年"
					+ string.split(" ")[0].split("-")[1] + "月"
					+ string.split(" ")[0].split("-")[2] + "日";
    ...
    /**
     * 将当前时间转换成要展示的形式   2013-09-12 周几   农历多少
     *
     * @param calendar
     * @return
     */
    public static String getCurrentDayWeekChineseDay(Calendar calendar) {
        return changeToChineseDay(calendar)+"   " + getWeek(calendar)+ "   农历" + new CalendarUtils(calendar).getDay();
    }
     ...
     * @return
     */
    public static String getCurrentWeekChineseDay(Calendar calendar) {
    	if(!StringUtils.isNullOrEmpty(new CalendarUtils(calendar).getDay())){
    		 return getWeek(calendar)+ " 农历" + new CalendarUtils(calendar).getDay();
    	}else{
    		return "";
    	}
